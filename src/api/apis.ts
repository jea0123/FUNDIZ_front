...
const toQueryString = (params: Record<string, unknown>) => {
    ...
    Object.entries(params).forEach(([k, v]) => {
        ...
        if (Array.isArray(v)) {
            v.forEach(item => {
                if (item !== undefined && item !== null && item !== '' && item !== 'undefined' && item !== 'null')
...
export const endpoints = {
    ...
    // ==================== Creator API ====================
    getCreatorProjectList: (p: SearchCreatorProjectDto) => `creator/projects?${toQueryString({ page: p.page, size: p.size, perGroup: p.perGroup, projectStatuses: p.projectStatus && p.projectStatus.length ? p.projectStatus : undefined, rangeType: p.rangeType || undefined })}`,
