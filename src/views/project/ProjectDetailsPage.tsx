...
export function ProjectDetailsPage() {
  ...
  const setCartQty = useCallback(
    (rewardId: number, nextQty: number) => {
      setCart((prev) => {
        if (nextQty <= 0) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="relative mb-6">
            <ImageWithFallback src={thumbnailUrl} alt={project.title} className="w-full h-96 object-cover rounded-lg" />
            <div className="absolute top-4 right-4 flex space-x-2">
              <Button variant="secondary" size="sm" disabled={loadingLike || mutatingLike} onClick={() => (isLiked ? dislikeProject(Number(projectId)) : likeProject(Number(projectId)))} className={isLiked ? 'text-red-500' : ''}>
                {loadingLike || mutatingLike ? <Loader2 className="h-4 w-4 animate-spin" /> : <Heart className={`h-4 w-4 ${isLiked ? 'fill-current' : ''}`} />}
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center space-x-3 p-4 bg-gray-50 rounded-lg">
              ...
              {loginUser?.creatorId === project.creatorId ? null : isFollowed ? (
                ...
                <Button variant="outline" size="sm" disabled={loadingFollow || mutatingFollow} onClick={() => followCreator(project.creatorId)} className="border-blue-500 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-950">
          ...
          <Tabs defaultValue="description" className="mb-8" onValueChange={(v) => setTab(v as any)}>
            <TabsList className="grid w-full grid-cols-4 gap-2">
              ...
              <TabsTrigger value="news" className="min-w-0 flex items-center justify-center gap-1 truncate">
                ...
                <span className="ml-1 inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-600">{project.newsList.length}</span>
              </TabsTrigger>
              ...
              <TabsTrigger value="community" className="min-w-0 flex items-center justify-center gap-1 truncate">
                ...
                <span className="ml-1 inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-600">{communityCnt}</span>
              </TabsTrigger>
              ...
              <TabsTrigger value="review" className="min-w-0 flex items-center justify-center gap-1 truncate">
                ...
                <span className="ml-1 inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-600">{reviewCnt}</span>
              </TabsTrigger>
            </TabsList>
                ...
                <>
                  {project.newsList.map((news) => (
                    <div key={news.newsId} className="space-y-4 mt-6">
                      <Card>
                        <CardContent>
                          <div className="whitespace-pre-wrap break-all [overflow-wrap:anywhere]">{news.content}</div>
            ...
            <TabsContent value="community">
              <ProjectCommunityTab projectId={Number(projectId)} active={tab === 'community'} onCreated={() => incCommunity(+1)} />
            </TabsContent>
        ...
        <div className="lg:col-span-1">
          <div className="space-y-6">
            ...
            <Card ref={cartRef} className={`bt-white transition ${cartPing ? 'ring-2 ring-blue-500/50 shadow-md' : ''}`}>
              <CardContent className="p-4 space-y-4">
                {Object.keys(cart).length === 0 ? (
                  ...
                  <>
                        ...
                        return (
                          <div key={rid} className="flex items-start justify-between gap-2">
                            ...
                            <div className="flex items-center gap-2">
                              <Button variant="outline" size="icon" onClick={() => setCartQty(rid, qty - 1)} disabled={qty <= 1}>
                                -
                              </Button>
                              <span className="w-6 text-center">{qty}</span>
                              <Button variant="outline" size="icon" onClick={() => setCartQty(rid, Math.min(max, qty + 1))} disabled={qty >= max || soldOut}>
                                +
                              </Button>
                    ...
                    <Button className="w-full" size="lg" onClick={backThisProject} disabled={cartSummary.totalQty === 0}>
            ...
            {project.rewardList.map((reward) => {
              ...
              return (
                <Card key={reward.rewardId} className={`transition-colors ${soldOut ? 'opacity-60 pointer-events-none' : ''}`}>
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="icon" onClick={() => decQty(reward.rewardId)} disabled={soldOut}>
                          -
                        </Button>
                        <span className="min-w-8 text-center">{Math.min(inputQty, Math.max(1, max))}</span>
                        <Button variant="outline" size="icon" onClick={() => incQty(reward.rewardId, max)} disabled={soldOut || inputQty >= max}>
                          +
                        </Button>
                      </div>
                      <Button onClick={() => addToCart(reward, Math.min(inputQty, Math.max(1, max)))} disabled={soldOut}>
                        담기
                      </Button>
                    </div>
                  </CardContent>
