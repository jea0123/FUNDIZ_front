...
export default function BackingTab() {
  ...
  const fetchBackingdetail = async (userId: number, projectId: number, rewardId: number, backingId: number) => {
    const cleanId = parseInt(backingId as any);
    const url = endpoints.getBackingDetail(userId, projectId, rewardId, cleanId);
    console.log('📡 요청 URL:', url);
    const response = await getData(endpoints.getBackingDetail(userId, projectId, rewardId, backingId));
  ...
  const openBackingById = (backingId: number) => {
    ...
    // 타입 안전 비교 (숫자 ↔ 문자열 방지)
    const target = backingProjects.find((b) => Number(b.backing.backingId) === Number(backingId));

    if (!target) {
      console.warn('❌ 해당 backingId에 대한 데이터를 찾을 수 없습니다:', backingId);
      console.log(
        '🔍 현재 backingProjects:',
    ...
    // 기존 함수 호출
    fetchBackingdetail(tempUserId, Number(target.backingReward.backingProject.projectId), Number(target.backingReward.rewardId), Number(target.backing.backingId));
  };
              ...
              <div key={`${backingList.backingReward.backingProject.projectId}-${index}`} className="flex items-center space-x-4 p-4 border rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-4 text-sm">
                    <span>후원금액: {formatNumber(backingList.backing.amount)}원</span>
                ...
                <Button variant="outline" size="sm" onClick={() => openBackingById(Number(backingList.backing.backingId))}>
                  상세보기
                </Button>
              </div>
            ))}
          ...
          <DialogContent className="w-screen h-screen min-w-350 max-w-none p-6">
            ...
            <div className="h-full overflow-y-auto space-y-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  달성금액 {formatNumber(selectedBacking.backingReward.backingProject.currAmount ?? 0)}원 / 목표 {formatNumber(selectedBacking.backingReward.backingProject.goalAmount ?? 0)}원
                </p>
              </div>
            </div>
            ...
            <section className="mt-4">
              ...
              <div className="text-sm space-y-1">
                ...
                <p>총 후원 금액 : {formatNumber(selectedBacking.backing.amount)}원</p>
              </div>
            </section>
